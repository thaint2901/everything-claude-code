...
| `ecc-statusline.js` shows the 5-hour and 7-day rate-limit windows plus the effort level, not dollar cost or session tool/file/duration counters — `// LOCAL (thaint):` at `buildMetricsSegment`, windows rendered by `scripts/lib/rate-limit-format.js`, `refreshInterval: 20` (seconds) added to the `statusLine` patch so it updates on a timer — 20s rather than 5s, since a shorter interval visibly jitters cost/context-window% during heavy background subagent activity (Claude Code gives statusLine no way to distinguish a timer tick from a real turn boundary). That cost fallback is model-aware: Claude Code's native `cost.total_cost_usd` for Anthropic models (it applies the 1h-cache and >200K tier multipliers a transcript sum cannot), the bridge's figure only for models `cost-tracker.js` has real rates for (DeepSeek), and no figure at all for any other model — Claude Code prices a gateway model at whichever Anthropic tier was requested, measured ~70x the real DeepSeek cost, so a missing number beats a wrong one | Keep; preserve the model-aware cost fallback (native / bridge-only / omitted), not a bare `nativeCost || bridgeCost` |
