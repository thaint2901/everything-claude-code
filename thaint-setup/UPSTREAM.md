...
| `ecc-statusline.js` shows the 5-hour and 7-day rate-limit windows plus the effort level, not dollar cost or session tool/file/duration counters — `// LOCAL (thaint):` at `buildMetricsSegment`, windows rendered by `scripts/lib/rate-limit-format.js`, `refreshInterval: 5` (seconds) added to the `statusLine` patch so it updates on a timer | Keep; preserve the fallback to cost when `rate_limits` is absent |
