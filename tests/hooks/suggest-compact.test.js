...
function runTests() {
  ...
  if (
    test('creates counter file on first run', () => {
  ...
  if (
    test('increments counter on subsequent runs', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('suggests compact at threshold (COMPACT_THRESHOLD=3)', () => {
      ...
      assert.ok(result.stderr.includes('3 tool calls reached') || result.stderr.includes('consider /compact'), `Should suggest compact at threshold. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('does NOT suggest compact before threshold', () => {
      const { sessionId, cleanup } = createCounterContext();
      cleanup();
      ...
      assert.ok(!result.stderr.includes('StrategicCompact'), 'Should NOT suggest compact before threshold');
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('suggests at threshold + 25 interval', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      ...
      assert.ok(result.stderr.includes('28 tool calls') || result.stderr.includes('checkpoint'), `Should suggest at threshold+25 interval. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('uses default threshold (50) when COMPACT_THRESHOLD is not set', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      ...
      assert.ok(result.stderr.includes('50 tool calls reached'), `Should use default threshold of 50. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('ignores invalid COMPACT_THRESHOLD (negative)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId, COMPACT_THRESHOLD: '-5' });
      // Invalid threshold falls back to 50
      assert.ok(result.stderr.includes('50 tool calls reached'), `Should fallback to 50 for negative threshold. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('ignores non-numeric COMPACT_THRESHOLD', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId, COMPACT_THRESHOLD: 'abc' });
      // NaN falls back to 50
      assert.ok(result.stderr.includes('50 tool calls reached'), `Should fallback to 50 for non-numeric threshold. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('resets counter on corrupted file content', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('resets counter on extremely large value', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('handles empty counter file', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('uses separate counter files per session ID', () => {
  ...
  if (
    test('always exits 0 (never blocks Claude)', () => {
      const { sessionId, cleanup } = createCounterContext();
      cleanup();
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId });
  ...
  if (
    test('rejects COMPACT_THRESHOLD=0 (falls back to 50)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      ...
      assert.ok(result.stderr.includes('50 tool calls reached'), `Should fallback to 50 for threshold=0. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('accepts COMPACT_THRESHOLD=10000 (boundary max)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      ...
      assert.ok(result.stderr.includes('10000 tool calls reached'), `Should accept threshold=10000. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('rejects COMPACT_THRESHOLD=10001 (falls back to 50)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      ...
      assert.ok(result.stderr.includes('50 tool calls reached'), `Should fallback to 50 for threshold=10001. Got stderr: ${result.stderr}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('rejects float COMPACT_THRESHOLD (e.g. 3.5)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      ...
      assert.ok(!result.stderr.includes('StrategicCompact'), 'Float threshold should be parseInt-ed to 3, no suggestion at count=50');
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('counter value at exact boundary 1000000 is valid', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('counter value at 1000001 is clamped (reset to 1)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
  ...
  if (
    test('emits hookSpecificOutput.additionalContext on stdout at threshold', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId });
      ...
      assert.strictEqual(parsed.hookSpecificOutput.hookEventName, 'PreToolUse', `hookEventName should be PreToolUse. Got: ${JSON.stringify(parsed)}`);
      assert.ok(parsed.hookSpecificOutput.additionalContext.includes('50 tool calls reached'), `additionalContext should include threshold text. Got: ${parsed.hookSpecificOutput.additionalContext}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('emits hookSpecificOutput.additionalContext on stdout at +25 interval', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      ...
      assert.ok(parsed.hookSpecificOutput.additionalContext.includes('28 tool calls'), `additionalContext should include count. Got: ${parsed.hookSpecificOutput.additionalContext}`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('emits no stdout below threshold (silent)', () => {
      const { sessionId, cleanup } = createCounterContext();
      cleanup();
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId, COMPACT_THRESHOLD: '5' });
      assert.strictEqual(result.code, 0);
      assert.strictEqual(result.stdout.trim(), '', `Expected empty stdout below threshold. Got: "${result.stdout}"`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('still writes [StrategicCompact] to stderr (debug log retained)', () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      fs.writeFileSync(counterFile, '49');
      const result = runCompact({ CLAUDE_SESSION_ID: sessionId });
      assert.ok(result.stderr.includes('[StrategicCompact]'), `stderr should retain [StrategicCompact] for debug log capture. Got: "${result.stderr}"`);
      cleanup();
    })
  )
    passed++;
  ...
  if (
    test('uses "default" session ID when CLAUDE_SESSION_ID is empty', () => {
      const defaultCounterFile = getCounterFilePath('default');
      try {
        fs.unlinkSync(defaultCounterFile);
      } catch (_err) {
        /* ignore */
      }
      try {
  ...
  if (
    test('malformed session IDs are not collapsed into the shared "default" counter', () => {
      const counterA = getCounterFilePath('ab');
      const counterB = getCounterFilePath('cd');
      const counterDefault = getCounterFilePath('default');
      [counterA, counterB, counterDefault].forEach(f => {
        try {
          fs.unlinkSync(f);
        } catch (_err) {
          /* ignore */
      ...
      try {
        const resultA = runCompactWithInput({ session_id: 'a/b' });
        const resultB = runCompactWithInput({ session_id: 'c/d' });
        assert.strictEqual(resultA.code, 0, 'Should exit 0');
        assert.strictEqual(resultB.code, 0, 'Should exit 0');
        assert.ok(fs.existsSync(counterA), 'session_id "a/b" should strip to its own counter file "ab"');
        assert.ok(fs.existsSync(counterB), 'session_id "c/d" should strip to its own counter file "cd"');
        assert.ok(!fs.existsSync(counterDefault), 'malformed but non-empty session IDs must not collapse into "default"');
        assert.strictEqual(parseInt(fs.readFileSync(counterA, 'utf8').trim(), 10), 1, 'Counter "ab" should be independent');
        assert.strictEqual(parseInt(fs.readFileSync(counterB, 'utf8').trim(), 10), 1, 'Counter "cd" should be independent');
      } finally {
        [counterA, counterB, counterDefault].forEach(f => {
          try {
            fs.unlinkSync(f);
  ...
  if (
    test('removes counter files older than retention window', () => {
      const { sessionId, cleanup } = createCounterContext();
      ...
      try {
        const result = runCompact({ CLAUDE_SESSION_ID: sessionId });
        assert.strictEqual(result.code, 0, 'Should exit 0');
        assert.ok(!fs.existsSync(stale), `Stale counter file should have been swept. Path: ${stale}`);
      } finally {
        try {
          fs.unlinkSync(stale);
        } catch (_err) {
  ...
  if (
    test('preserves counter files within retention window', () => {
      const { sessionId, cleanup } = createCounterContext();
      ...
      try {
        runCompact({ CLAUDE_SESSION_ID: sessionId });
        assert.ok(fs.existsSync(fresh), `Fresh counter file should be preserved. Path: ${fresh}`);
      } finally {
        try {
          fs.unlinkSync(fresh);
        } catch (_err) {
  ...
  if (
    test("preserves the active session's counter file even if old", () => {
      const { sessionId, counterFile, cleanup } = createCounterContext();
      cleanup();
      ...
      try {
        ...
        assert.ok(fs.existsSync(counterFile), 'Active session counter must survive the sweep');
        const count = parseInt(fs.readFileSync(counterFile, 'utf8').trim(), 10);
        assert.strictEqual(count, 8, `Active counter should be incremented by the hook. Got ${count}`);
      } finally {
        cleanup();
      }
    })
  ...
  if (
    test('honours COMPACT_STATE_TTL_DAYS env var', () => {
      const { sessionId, cleanup } = createCounterContext();
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(!fs.existsSync(target), `TTL=3 should sweep a 5-day-old file. Path: ${target}`);
      } finally {
        try {
  ...
  if (
    test('falls back to default for invalid COMPACT_STATE_TTL_DAYS', () => {
      const { sessionId, cleanup } = createCounterContext();
      ...
      try {
        ...
        for (const bad of ['0', '-5', 'abc']) {
          ...
          assert.strictEqual(result.code, 0);
          assert.ok(fs.existsSync(target), `Invalid TTL '${bad}' should fall back to default (14d) and preserve a 5-day-old file`);
        }
      } finally {
        try {
          fs.unlinkSync(target);
  ...
  if (
    test('does not touch unrelated temp files', () => {
      const { sessionId, cleanup } = createCounterContext();
      ...
      try {
        runCompact({ CLAUDE_SESSION_ID: sessionId });
        assert.ok(fs.existsSync(unrelated), `Unrelated temp file should not be swept. Path: ${unrelated}`);
      } finally {
        try {
  ...
  if (
    test('preserves files whose mtime sits at or after the TTL cutoff', () => {
      ...
      try {
        const result = runCompact({ CLAUDE_SESSION_ID: sessionId });
        assert.strictEqual(result.code, 0);
        assert.ok(fs.existsSync(boundary), `Boundary-aged counter file should be preserved. Path: ${boundary}`);
      } finally {
        try {
  ...
  if (
    test('exit 0 holds when sweep encounters a populated temp dir', () => {
  ...
  if (
    test('suggests compact when context exceeds the 200k-window threshold', () => {
  ...
  if (
    test('stays silent below the context threshold', () => {
      const ctx = createContextContext();
  ...
  if (
    test('honours COMPACT_CONTEXT_THRESHOLD override', () => {
      ...
      try {
        const result = runCompactWithInput({ session_id: ctx.sessionId, transcript_path: transcript }, { COMPACT_CONTEXT_THRESHOLD: '1000' });
  ...
  if (
    test('does not re-fire within the same context bucket', () => {
      const ctx = createContextContext();
      const transcript = writeTranscriptFixture(170000);
      try {
  ...
  if (
    test('re-fires after the context grows by another interval', () => {
      const ctx = createContextContext();
      const transcript = writeTranscriptFixture(170000);
      try {
  ...
  if (
    test('uses the 250k default threshold for [1m] models', () => {
      const ctx = createContextContext();
  ...
  if (
    test('prefers the bridge-derived window over the static model table', () => {
      const ctx = createContextContext();
      ...
      try {
        ...
        try {
          fs.unlinkSync(getBridgePath(ctx.sessionId));
        } catch (_err) {
          /* ignore */
        }
        ctx.cleanup();
  ...
  if (
    test('COMPACT_CONTEXT_THRESHOLD=0 disables the context signal', () => {
      const ctx = createContextContext();
      const transcript = writeTranscriptFixture(170000);
      try {
        const result = runCompactWithInput({ session_id: ctx.sessionId, transcript_path: transcript }, { COMPACT_CONTEXT_THRESHOLD: '0' });
  ...
  if (
    test('survives a malformed transcript (exit 0, silent)', () => {
      const ctx = createContextContext();
  ...
  if (
    test('survives a missing transcript path (exit 0, count signal intact)', () => {
      const ctx = createContextContext();
      try {
  ...
  if (
    test('emits a single stdout JSON payload when both signals fire', () => {
      const ctx = createContextContext();
      const transcript = writeTranscriptFixture(170000);
      try {
        fs.writeFileSync(ctx.counterFile, '49');
  ...
  if (
    test('sweeps stale context bucket state files', () => {
      const ctx = createContextContext();
